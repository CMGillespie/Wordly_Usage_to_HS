...
# wordly_unified_master_v3.2a_CLOUD.py
# VERSION: 3.2a-CLOUD
# CHANGE FROM v3.2-CLOUD:
#   - Reverted wait_for_selector back to wait_for_timeout(5000) in service loop
#     (wait_for_selector caused silent skips in headless cloud environment)
...
def run_v3_2a():
    print(f"🚀 Launching v3.2a-CLOUD at {datetime.now().strftime('%H:%M:%S')}")
    ...
    if downloaded_files:
        ...
        if os.path.exists(HS_KEY_FILE):
            ...
            # Companies — includes hubspot_owner_id
        ...
        # CSV — strip owner columns, keep everything else unchanged
        ...
        send_slack_message(f"v3.2a-CLOUD Success. Rows: {len(master_df)}", is_error=False)
