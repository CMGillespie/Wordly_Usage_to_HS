...
# wordly_unified_master_v3.2_LOCAL.py
# VERSION: 3.2-LOCAL
# CHANGES FROM v3.1.5-CLOUD:
#   - Paths set to local Mac/Google Drive environment for test run
#   - Added fetch_hs_owner_map() — single call, builds {owner_id: "First Last"} dict
#   - Added hubspot_owner_id to company fetch properties
#   - Added hs_account_owner column to BQ payload (stripped from CSV)
#   - Stripped raw hs_owner_id from both CSV and BQ
...
# Credentials live in the same folder as the script
SCRIPT_DIR = BASE_DIR
...
def fetch_hs_owner_map(token):
    """One call. Returns dict of {owner_id_string: 'First Last'}"""
...
def run_v3_2():
    print(f"🚀 Launching v3.2-LOCAL at {datetime.now().strftime('%H:%M:%S')}")
    ...
    with sync_playwright() as p:
        # headless=False for local — lets you see what's happening
    ...
    if downloaded_files:
        ...
        if os.path.exists(HS_KEY_FILE):
            ...
            # Companies — now includes hubspot_owner_id
            ...
            # One call to resolve all owner IDs to names
            ...
            # Resolve owner name — BQ only, not in CSV
        ...
        # CSV — strip owner columns, keep everything else as before
        ...
        try:
            ...
            # BQ gets hs_account_owner but NOT raw hs_owner_id
            ...
            # Sanity check — print owner column to confirm it populated
            print("\n--- OWNER SANITY CHECK ---")
            print(bq_df[['Owner_Email', 'hs_account_owner']].drop_duplicates().head(20).to_string())
            print("--- END OWNER CHECK ---\n")

        except Exception as e:
            print(f"⚠️ BigQuery Mirroring failed, but CSV is safe! Error: {e}")

        print("DEBUG COLUMNS:", list(master_df.columns))


        # Sanity check — outside BQ try/catch so it always runs
        print("\n--- OWNER SANITY CHECK ---")
        owner_check = master_df[['Owner Email', 'hs_account_owner']].drop_duplicates()
        print(f"Rows with owner name populated: {owner_check['hs_account_owner'].ne('').sum()} / {len(owner_check)}")
        print(owner_check[owner_check['hs_account_owner'].ne('')].head(10).to_string())
        print("--- END OWNER CHECK ---\n")

        send_slack_message(f"v3.2-LOCAL Success. Rows: {len(master_df)}", is_error=False)
